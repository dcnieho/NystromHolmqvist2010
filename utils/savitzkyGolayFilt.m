...
%
%   savitzkyGolayFilt(X,B) same as above, but takes the coefficients
%   obtained from savitzkyGolay() as input instead. It is assumed that
%   filter is symmetric and that coefficients for startpoint:endpoint are
%   provided, otherwise function might not return correct results.
%
%   savitzkyGolayFilt(X,N,DN,F,[],DIM), savitzkyGolayFilt(X,N,DN,F,W,DIM)
%   or savitzkyGolayFilt(X,B,DIM) operates along the dimension DIM.
%
...
%
%   Originally based on (extended to support differentiation filters, which
%   required some changes to the main algorithm in this function as well)
%       m-file: sgolayfilt.m
%       Author(s): R. Losada
