function [fc, df] = savitzkyGolayMult(x,n,dn,x0,W,flag)
% Function:
...
%       n    = polynomial order
%       dn   = differentation order (0=smoothing),
%              can be vector, default=0
...
%
%   output:
%       fc   = filter coefficients obtained (B output of sgolay). Cell
%              array with one entry per specified dn.
...
% Example:
%       savitzkyGolayMult([-3:3],2,0,0,[],0)
%       savitzkyGolayMult([-3:3],2,[0 1 2],0,[],0)
% Author:
...
%
% See also:
%       sgolay, savitzkyGolay, savitzkyGolayFiltMult
...
if max(dn) > n, error(generatemsgid('InvalidRange'),'The Differentiation order must be less than or equal to the Polynomial order.'), end
...
for p=1:length(dn)
    hx = [(zeros(Nx0,dn(p))) ones(Nx0,1)*prod(1:dn(p))];    % order=0:dn-1,& dn,respectively
    for k=1:n-dn(p)                                         % order=dn+1:n=dn+k
        hx = [hx x0.^k*prod(dn(p)+k:-1:k+1)];
    end
