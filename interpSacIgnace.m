function [data] = interpSacIgnace(data,order,start1,fin1,start2,fin2,isaccsta,isaccend,qDebug)

% isaccsta is index laatste sample voor de saccade
% isaccend is index eerste  sample na   de saccade
...
t1 = isaccsta;                          % start en eind punt van de interpolatie
t2 = isaccend - 1;
...
if 1
    ...
    % data nul is.
    new_t = [isaccsta-1:isaccend];
    newdata = b0 + b1*new_t + b2*new_t.^2 + b3*new_t.^3;
    
    % corrigeer offsets aan begin en einde van saccade
    offstart =  newdata(1  )-data(isaccsta-1);
    offeind  = -newdata(end)+data(isaccend)+offstart;       % dit is gelijk aan de saccade amplitude!
    % hang geinterpoleerde data erin op de plaats van de saccade
    data(isaccsta:isaccend-1)   = newdata(2:end-1)   - offstart;
    data(isaccend:end)          = data(isaccend:end) - offeind;
else
    % OUDE CODE: gebruik je dit dan heb je een plateau (twee
    % dezelfde oogposities) net voor en na je saccade
    
    % hang geinterpoleerde data erin op de plaats van de saccade
    new_t = [isaccsta:isaccend-1];
    data(isaccsta:isaccend-1) = b0 + b1*new_t + b2*new_t.^2 + b3*new_t.^3;
    
    % corrigeer offsets aan begin en einde van saccade
    data(isaccsta:end) = data(isaccsta:end) - (data(isaccsta) - data(isaccsta-1));
    data(isaccend:end) = data(isaccend:end) - (data(isaccend) - data(isaccend-1));
end
