...
function [X,Y,vel,velX,velY] = cutPursuitImplementation(X,Y,vel,velX,velY, sac,ETparams,skipFirstWindow)
...
for p=1:length(pur.on)
    ...
    [vel,velX,velY] = replaceIntervalVelocity(vel,velX,velY,pur.on(p),pur.off(p),0);
end
