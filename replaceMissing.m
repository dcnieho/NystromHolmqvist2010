function data = replaceMissing(data,qInterpMissingPos)
% Currently, this only replaces missing data with linear interpolation for
% the velocity traces. It should be straightforward to implement if you
% need this for position or other traces.

% process data
[data.deg.missing,...
 data.deg.Azi,...
 data.deg.Ele,...
 data.deg.vel,...
 data.deg.velAzi,...
 data.deg.velEle] = replaceMissingImplementation(data.deg.Azi,...
                                                 data.deg.Ele,...
                                                 data.deg.vel,...
                                                 data.deg.velAzi,...
                                                 data.deg.velEle,...
                                                 true,...
                                                 data.deg.missing,...
                                                 qInterpMissingPos,true);

if isfield(data.pix,'vel')
    [data.pix.missing,...
     data.pix.X,...
     data.pix.Y,...
     data.pix.vel,...
     data.pix.velX,...
     data.pix.velY] = replaceMissingImplementation(data.pix.X,...
                                                   data.pix.Y,...
                                                   data.pix.vel,...
                                                   data.pix.velX,...
                                                   data.pix.velY,...
                                                   false,...
                                                   data.pix.missing,...
                                                   qInterpMissingPos,false);
elseif qInterpMissingPos
     [data.pix.missing,...
     data.pix.X,...
     data.pix.Y]    = replaceMissingImplementation(data.pix.X,...
                                                   data.pix.Y,...
                                                   [],...
                                                   [],...
                                                   [],...
                                                   false,...
                                                   data.pix.missing,...
                                                   qInterpMissingPos,false);
end




function [missing,X,Y,vel,velX,velY] = replaceMissingImplementation(X,Y,vel,velX,velY,qDeg,missing,qInterpMissingPos,qPrintInfo)

% We want to deal with all the nan in the data.
% This is getting rid of blinks and such...
if ~isempty(vel)
    qNaN = isnan(vel);
else
    qNaN = isnan(X);
end
if any(qNaN)
    [nanon,nanoff] = bool2bounds(qNaN);
    assert(all(missing.on==nanon) && all(missing.off==nanoff))
    % gooi NaNnen gevonden aan begin en einde trial eruit, daar kunnen we
    % niets mee...
    if nanon(1)==1
        nanon(1)    = [];
        nanoff(1)   = [];
    end
    if ~isempty(nanoff) && nanoff(end)==length(X)       % might be empty by now...
        nanon(end)  = [];
        nanoff(end) = [];
    end
    
    % adjust indices, nanon(p) and nanoff(p) point to first and last NaN in
    % a run
    nanon  = nanon-1;
    nanoff = nanoff+1;
    nanlen = nanoff-nanon+1;
    
    for p=1:length(nanon)
        % replace with interpolated velocity
        if ~isempty(vel)
            [vel,velX,velY] = replaceIntervalVelocity(vel,velX,velY,Y,qDeg,nanon(p),nanoff(p));
        end
        
        if qInterpMissingPos
            X(nanon(p):nanoff(p)) = linspace(X(nanon(p)), X(nanoff(p)), nanlen(p));
            Y(nanon(p):nanoff(p)) = linspace(Y(nanon(p)), Y(nanoff(p)), nanlen(p));
        end
    end
    
    % get info about how much still missing
    qNaN = isnan(X);
    [nanon,nanoff] = bool2bounds(qNaN);
    missing.leftover.on  = nanon;
    missing.leftover.off = nanoff;
    
    if qPrintInfo
        % show how many NaN we have left now, those cannot be handled
        % (first message of how many missing is issued in flagMissing.m)
        fprintf('   -> N NaN samples left: %d (%.2f%%)\n',sum(qNaN),sum(qNaN)./length(X)*100);
    end
end
