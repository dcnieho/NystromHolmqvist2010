...
% amplitude - TODO replace with cosine rule
data.(fieldname).amplitude ...
    = hypot(...
            data.deg.X(data.(fieldname).off) - data.deg.X(data.(fieldname).on), ...
            data.deg.Y(data.(fieldname).off) - data.deg.Y(data.(fieldname).on)  ...
           ).';
